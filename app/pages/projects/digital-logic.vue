<template>
  <div>
    <el-form @submit.prevent inline>
      <el-text size="small" class="block">
        <em>
          3
          <sup>n</sup>
          possible implicants will be tried
        </em>
      </el-text>
      <el-text size="small" class="block">
        <em>
          DFS used for finding essential prime implicants with max 10
          <sup>5</sup>
          iterations.
        </em>
        <el-tag class="mx-2" v-if="cover.iterations">
          {{ cover.iterations }}
        </el-tag>
      </el-text>
      <el-form-item label="Logic Expression">
        <el-input
          v-model="form.logicExpr"
          placeholder="!(a + b) == !c && true || false">
          <template #suffix>
      ...
      <el-form-item label="Variable Count">
        <el-select
          v-model="form.varCount"
          placeholder="Input Size"
          :disabled="Boolean(form.logicExpr)">
          <el-option
            v-for="i in 5"
            :key="i"
            :label="i + 1"
      ...
      <el-form-item>
        <template #label>
          <el-checkbox v-model="form.shouldFactor">
            Factorization (Shannon)
          </el-checkbox>
        </template>
        <el-input-number
          v-model="form.factorCount"
          placeholder="Factor Count"
          :min="1"
          :max="form.varCount - 1" />
      </el-form-item>
      <el-form-item label="Factors" v-if="factorized">
        <el-select v-model="form.factors" placeholder="Auto" clearable>
          <template #label>
            <span
              v-if="form.factors !== undefined"
              v-html="
                katex.renderToString(
                  factorCombs[form.factors].map((i) => varName(i)).join('')
                )
              "></span>
            <el-tag
              v-if="form.factors !== undefined && factorized.costsMap"
              class="ml-2">
              cost
              {{ factorized.costsMap.get(factorCombs[form.factors])?.cost }}
            </el-tag>
          </template>
          <el-option v-for="(factors, i) in factorCombs" :key="i" :value="i">
            <span
              v-html="
                katex.renderToString(factors.map((i) => varName(i)).join(''))
              "></span>
            <el-tag
              v-if="factorized.costsMap"
              class="ml-2"
              :type="
                factorized.costsMap.get(factors)?.cost === factorized.cost
                  ? 'primary'
                  : 'info'
              ">
              cost {{ factorized.costsMap.get(factors)?.cost }}
            </el-tag>
          </el-option>
        </el-select>
      </el-form-item>
      <el-form-item
        label="Expression"
        :error="formErrors['expr']"
        class="w-full">
        <el-input
          v-model="form.expression"
          placeholder="m(0,1,2,3)+d(4,5,6)"
          :disabled="Boolean(form.logicExpr)">
      ...
      <div class="flex gap-2">
        <el-tag
          type="danger"
          v-for="k in Object.keys(formErrors).filter((e) => formErrors[e])">
          {{ k }} : {{ formErrors[k] }}
        </el-tag>
        <el-tag
          type="info"
          v-for="k in Object.keys(timing).filter((e) => timing[e])"
          v-if="form.showTiming">
    ...
    <div class="flex gap-4 flex-col lg:flex-row items-center">
      <div ref="kmapTables" class="grid grid-cols-2 gap-1">
        <table v-for="(zmap, z) in kmap.rows" class="kmap relative">
          <thead>
            <tr v-if="kmap.depth > 1">
              ...
              <th
                ...
                v-html="
                  katex.renderToString(
                    varName(kmap.inputSize - 4) +
                      ' ' +
                      varName(kmap.inputSize - 3)
                  )
                "></th>
            </tr>
            <tr>
              <th
                v-html="
                  kmap.depth > 1
                    ? katex.renderToString(
                        varName(kmap.inputSize - 2) +
                          ' ' +
                          varName(kmap.inputSize - 1)
                      )
                    : ''
                "></th>
          ...
          <tbody>
            <tr v-for="(row, i) in zmap" :key="i">
              ...
              <td v-for="(cell, j) in row" :key="j">
                <ruby
                  @click="
                    updateExpr(cell.label, increm(truthTable.rows[cell.label]))
                  "
                  :class="'bit-' + cell.bit">
                  {{ cell.bit }}
                  <rt>{{ cell.label }}</rt>
          ...
          <div
            v-for="rect in implicantsRects.filter((imp) => imp.z === z)"
            :style="{
              position: 'absolute',
              pointerEvents: 'none',
              ...
              borderRight: rect.borders.right
                ? `1px solid ${rect.color}`
                : 'none',
              borderBottom: rect.borders.bottom
                ? `1px solid ${rect.color}`
                : 'none',
              borderLeft: rect.borders.left
                ? `1px solid ${rect.color}`
                : 'none',
            }"></div>
        </table>
      ...
      <div>
        <div class="flex gap-2">
          ...
          <el-tag type="danger" v-if="factorized">
            Cost (sum of factors):
            {{
              form.factors
                ? factorized.costsMap?.get(factorCombs[form.factors])?.cost
        ...
        <div class="implicants-katex" v-html="implicantKatex"></div>
      </div>
    </div>
    ...
    <el-table :data="truthTable.rows" :height="500">
      <el-table-column
        v-for="i in truthTable.inputSize"
        :key="i"
        align="center">
        <template #default="{ row, $index }">
          <el-tag
            v-if="$index & (1 << (truthTable.inputSize - i))"
            type="success">
            1
          </el-tag>
      ...
      <el-table-column align="center" class="max-w-full">
        <template #default="{ row, $index }">
          <el-tag v-if="row === '1'" type="success">1</el-tag>
          <el-tag v-else-if="row === '0'" type="danger">0</el-tag>
...
import {
  ...
  optimizeFactorized,
  factorizedToLatex,
  type FactorizedCover,
  type Bit,
  type Cover,
  type KMap,
  type TruthTable,
} from "~/libraries/logic-expr";
import {
  generateTruthTable,
  parseLogicExpression,
} from "~/libraries/logic-expr/expression";
import {
  drawImplicants,
  type DrawImplicantRect,
} from "~/libraries/logic-expr/render";
definePageMeta({
  title: "Digital Logic",
...
function varName(i: number) {
  if (form.logicExpr)
    return vars.value[i]?.replace(/(\d+)$/, "_{$1}") ?? `x_{${i + 1}}`;
  else return `x_{${i + 1}}`;
}
...
function randomize() {
  ...
  const rows: Bit[] = Array.from({ length: 2 ** form.varCount }, () =>
    Math.random() > 0.5 ? "1" : "0"
  );
  if (includeDC)
    for (let i = 0; i < rows.length; i++)
      if (Math.random() > 0.8) rows[i] = "x";
  form.expression = toExpression(rows);
}
...
const cover = computedAsync<Cover>(
  async () => {
    try {
  ...
  (cover) => {
    ...
    setTimeout(() => {
      try {
        ...
        implicantsRects.value = drawImplicants(
          kmapTables.value!.children,
          kmap.value.width,
          kmap.value.height,
          cover.implicants.filter((_, i) => cover.included[i])
        );
        timing["implicantsRects"] =
          performance.now() - timing["implicantsRects"];
      } catch (e: any) {
  ...
  try {
    ...
    if (form.shouldFactor && factorized.value) {
      if (form.factors !== undefined) {
        const fcover = factorized.value.costsMap?.get(
          factorCombs.value[form.factors]
        );
    ...
    const html =
      ...
      (form.showLatex
        ? "<span class='block font-mono text-xs max-w-4xl'>$$<br/>" +
          latex +
          "<br/>$$</span>"
        : "");
