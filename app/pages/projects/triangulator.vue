      ...
      <table class="px-2">
        ...
        <tbody>
          <tr v-for="(anchor, index) in anchors" :key="index">
            <td>
              <el-checkbox
                v-model="anchor.disabled"
                :label="
                  index +
                  1 +
                  (selectedAnchor === anchor || selectedAnchorLine == anchor
                    ? ' ←'
                    : '')
                " />
            </td>
          ...
          <tr>
            ...
            <td>
              <el-button @click="anchors.push({ ...newAnchor })">+</el-button>
            </td>
          </tr>
        </tbody>
      </table>
      <el-form label-position="top" inline v-if="showOptions" class="px-2">
        ...
        <el-form-item label="Error Graph">
          <el-select v-model="errorGraph">
            <el-option
              v-for="item in errorGraphOptions"
              :key="item.label"
              :label="item.label"
              :value="item.value" />
          </el-select>
...
function totalAngularErrorSquared(
  x: number,
  y: number,
  anchors: Anchor[]
): number {
  return anchors.reduce(
    (sum, anchor) => sum + angularErrorSquared(x, y, anchor),
    0
  );
}
function getAABB(anchors: Anchor[]): {
  minX: number;
  minY: number;
  maxX: number;
  maxY: number;
...
function gradientDescent(
  ...
  for (; iteration < maxIterations; iteration++) {
    ...
    let gx = anchors.reduce((sum, anchor) => {
      ...
      error = Math.atan2(Math.sin(error), Math.cos(error));
      let f =
        falloff === 0 ? 1 : 1 / Math.pow(Math.hypot(x - xi, y - yi), falloff);
    ...
    learningRate =
      error < prevError
        ? learningRate * rewardMultiplier.value
        : learningRate * errorMultiplier.value;
...
function radiusOfProbability(x: number, y: number, anchors: Anchor[]): number {
  const distances = anchors.map((anchor) =>
    perpendicularDistance(x, y, anchor)
  );
...
function drawErrorGraph(ctx: CanvasRenderingContext2D) {
  ...
  const maxError = Math.max(
    ...iterationHistory.value.map((h) => trans(h.error))
  );
  ...
  ctx.beginPath();
  ctx.moveTo(0, 50 - norm(trans(iterationHistory.value[0]?.error ?? 0)) * 50);
  iterationHistory.value.forEach((h, i) => {
    ctx.lineTo(
      (i * 100) / iterationHistory.value.length,
      50 - 50 * norm(trans(h.error))
    );
  });
  ctx.stroke();
  ctx.restore();
...
function startRender() {
  ...
  drawErrorGraph(ctx);
  ctx.lineWidth = 2;
  const { fore, anchor, anchorSelect, anchorHover, anchorDisable, target } =
    colors.value;
  anchors.forEach((a) => {
    ...
    ctx.strokeStyle =
      (a.disabled
        ? anchorDisable
        : a === selectedAnchorLine.value
          ? anchorSelect
          : a === hoverAnchorLine.value
            ? anchorHover
            : anchor) + "9";
    let stroke = false;
    if (tx < 0 || tx > ctx.canvas.width || ty < 0 || ty > ctx.canvas.height) {
      stroke = true;
      const i = findIntersection(
        ctx.canvas.width,
        ctx.canvas.height,
        tx,
        ty,
  ...
  ctx.fillText(
    `(${displayNumber(result.value.x)}, ${displayNumber(result.value.y)})`,
    rtx,
    rty - 10
  );
  rescale();
...
function tranC2W(x: number, y: number): [number, number] {
  return [
    (x - canvasOpt.ox) / canvasOpt.scale,
    (-y + canvasOpt.oy) / canvasOpt.scale,
  ];
}
function tranW2C(x: number, y: number): [number, number] {
  return [
    x * canvasOpt.scale + canvasOpt.ox,
    -y * canvasOpt.scale + canvasOpt.oy,
  ];
}
...
function getAnchor(x: number, y: number): Anchor | undefined {
  return aFiltered.value.find(
    (anchor) => hypot(tranW2C(anchor.x, anchor.y), [x, y]) < 5
  );
}
function getAnchorLine(x: number, y: number): Anchor | undefined {
  ...
  return aFiltered.value.find(
    (anchor) =>
      perpendicularDistance(tx, ty, anchor) * canvasOpt.scale < 5 &&
      (tx - anchor.x) * anchor.dx + (ty - anchor.y) * anchor.dy > 0
  );
}
function canvasDown(e: MouseEvent) {
  mouseDownPos.value = getxy(e);
  selectedAnchor.value = getAnchor(
    mouseDownPos.value[0],
    mouseDownPos.value[1]
  );
  if (!selectedAnchor.value)
    selectedAnchorLine.value = getAnchorLine(
      mouseDownPos.value[0],
      mouseDownPos.value[1]
    );
  ...
  if (mouseDownPos.value) {
    ...
    if (selectedAnchor.value) {
      ...
      const length = Math.hypot(
        selectedAnchorLine.value.dx,
        selectedAnchorLine.value.dy
      );
...
const iterationHistory = ref(
  <{ x: number; y: number; gx: number; gy: number; error: number }[]>[]
);
const radius = computed(() =>
  radiusOfProbability(result.value.x, result.value.y, aFiltered.value)
);
onMounted(() => {
