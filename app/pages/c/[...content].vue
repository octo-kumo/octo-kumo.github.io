...
const { data: alldocs } = await useLazyAsyncData(`c/docs`, () =>
  queryAllDocs()
);
const docs = computed(() => alldocs.value?.flat);
const navigation = computed(() => alldocs.value?.nav);

const nav: ComputedRef<{
  prev?: Partial<ContentNavigationItem>;
  next?: Partial<ContentNavigationItem>;
}> = computed(() => {
...
const contentSpacingRight = computed(() =>
  TOC.value.length > 0 ? "12.5rem" : "0"
);
...
const filterTreeNode: FilterNodeMethodFunction = (
  values: string[],
  data: TreeNodeData
) => {
...
<template>
  <template v-if="doc">
    <article class="content-page-sections mt-1">
      ...
      <el-anchor
        ...
        <table-of-contents
          v-for="child in TOC"
          :link="child"></table-of-contents>
      </el-anchor>
      <h1>
        <span
          class="mb-2 text-4xl mt-4 block font-bold font-mono article-title"
      ...
      <el-text size="small" class="block font-mono">
        <hover-text class="block">
          <template #default>
            <span
              v-shared="getTransitionName(doc, 'dates')"
      ...
      <div
        class="flex justify-between mt-3 print:hidden"
        v-shared="'content-page-peer-nav'"
    ...
    <lazy-comments
      class="print:hidden content-page-sections py-1"
      v-if="!isContentEmpty(doc)" />
    <el-divider
      v-shared="'content-tree-sep'"
      class="print:hidden mx--3!"
      style="width: calc(100% + 1.5rem)"
      v-if="!isContentEmpty(doc)" />
    <lazy-writeup-statistics
      class="content-page-sections mb-1"
      v-if="path === '/ctf' && docs"
      :docs="docs"
  ...
  <template v-if="docs && docs.length > 0">
    <el-tree
      class="text-base! content-page-sections mb-20 print:hidden"
      :current-node-key="path"
      node-key="path"
      ref="treeRef"
      :default-expand-all="false"
      v-shared="'content-tree-nav'"
      highlight-current
      auto-expand-parent
      :filter-node-method="filterTreeNode"
      :default-expanded-keys="[path]"
      :data="navigation as any"
      :props="treeProps">
      <template #default="{ node, data }">
        <span class="flex justify-between flex-1">
          <!--v-shared="getTransitionName(data, 'tree-node')"-->
          <kumo-link
            :id="'content_' + hashCode(data.path).toString(16).padStart(8, '0')"
            :to="'/c' + data.path"
            class="mr-2 justify-start!"
            no-prefetch
            :class="{ 'font-bold!': oneLvlUp(data.path) === '/ctf' }">
            <el-tooltip
              :show-after="500"
              effect="light"
              :hide-after="0"
  ...
  <el-skeleton
    v-if="status === 'pending'"
    v-shared="'content-loading-skeleton'" />
  <template v-else-if="!(doc || (docs && docs.length > 0))">
    <el-empty
      description="404 not found"
      class="h-48 flex-auto content-page-sections"
      :image-size="80"
