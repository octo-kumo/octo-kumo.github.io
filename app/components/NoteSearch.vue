<template>
  <el-input
    ...
    <template #append>
      <el-select v-model="searchMode" placeholder="Select" style="width: 115px">
        <el-option
          v-for="item in Object.keys(SEARCH_MODES)"
          :key="item"
          :label="item"
          :value="item" />
      </el-select>
  ...
  <div v-if="searchMode === 'Custom'" class="mt-2! flex flex-row gap-2">
    ...
    <el-button
      :type="getSortBtnType('points')"
      :icon="getSortBtnIcon('points')"
      class="mx-0!"
      @click="toggleSort('points')">
      Points
    </el-button>
    <el-button
      :type="getSortBtnType('solves')"
      :icon="getSortBtnIcon('solves')"
      class="mx-0!"
      @click="toggleSort('solves')">
      Solves
    </el-button>
  </div>
  <el-pagination
  ...
  <el-timeline class="mt-2!" v-auto-animate>
    <el-timeline-item
      v-for="{ item: doc, matches, score } in docsFiltered.slice(
        currPage * 5 - 5,
        currPage * 5
      )"
      :key="doc.path"
      ...
      <kumo-link
        ...
        <span
          v-html="
            highlight(
              doc.title,
              matches?.find((m) => m.key === 'title')?.indices
            )
          "></span>
      </kumo-link>
      <el-tag
        v-if="score"
        :type="score < 0.1 ? 'success' : score < 0.5 ? 'warning' : 'danger'"
        size="small"
        class="ml-1">
      ...
      <el-text class="font-mono" size="small" v-if="isSearching">
        <span
          v-html="
            '/c' +
            highlight(doc.path, matches?.find((m) => m.key === 'path')?.indices)
          "></span>
      </el-text>
      <el-text class="block mt-1!">
        <span
          v-html="
            highlight(
              doc.description,
              matches?.find((m) => m.key === 'description')?.indices
            )
          "></span>
      </el-text>
      <article-tags
        ...
        " />
      <span
        class="el-timeline-item__timestamp is-bottom font-mono"
...
const { data: alldocs, status: docStatus } = await useLazyAsyncData(
  `c/docs`,
  () => queryAllDocs()
);
const docs = computed(() =>
...
const getSortBtnType = (key: "points" | "solves") =>
  customSortKey.value !== key
    ? "default"
    : customSortDir.value === "desc"
      ? "primary"
      : "danger";
const getSortBtnIcon = (key: "points" | "solves") =>
  customSortKey.value !== key
    ? ElIconSort
    : customSortDir.value === "asc"
      ? ElIconSortUp
...
function customFilter(
  items: FuseResult<PageCollections["content"]>[]
): FuseResult<PageCollections["content"]>[] {
  if (searchMode.value !== "Custom") return items;
  items = items.filter((item) =>
    (filterTags.value ?? []).every((tag) => item.item?._tags?.includes(tag))
  );
  if (typeFilter.value.length)
    items = items.filter((item) =>
      typeFilter.value.some((i) => TYPES[i].filter.test(item.item.path))
    );
...
const fuse = computed(
  () => new Fuse(docs.value ?? [], SEARCH_MODES[searchMode.value])
);
watch(docs, () => fuse.value.setCollection((docs.value ?? []) as never[]), {
  immediate: true,
});
...
const docsFiltered = computed(() =>
  customFilter(
    isSearching.value
      ? fuse.value.search(search.value).filter((e) => (e?.score ?? 1) < 0.9)
      : (docs.value ?? []).map(
          (w) =>
            ({
              item: w,
