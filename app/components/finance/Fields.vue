    ...
    <el-radio-group
      ...
      <div
        ...
        <el-radio
          ...
          class="mr-0!"
          v-model="selectedRow"
          :value="field.key"></el-radio>
        <math-field
          v-model="values[field.key]"
          :readonly="selectedRow === field.key"
          :prefix="field.key + '=' + field.prefix"
          :suffix="field.suffix"></math-field>
      </div>
    </el-radio-group>
    <p
      v-if="calculationError.length > 0"
      v-text="calculationError.join('\n')"></p>
  </el-card>
</template>
...
export type FieldCalcMap = {
  [k: (typeof props.fields)[number]["key"]]: (values: {
    [k: (typeof props.fields)[number]["key"]]: number;
  }) => number;
};
...
const values = reactive(
  Object.fromEntries(props.fields.map((field) => [field.key, 0]))
);
const selectedRow: Ref<(typeof props.fields)[number]["key"]> = ref(
  props.fields.findLast((e) => !e.computed)!.key!
);
