...
function getMask(): Promise<ArrayBuffer | undefined> {
  return new Promise((resolve, reject) =>
    canvas.value!.toBlob(async (blob: Blob | null) => {
      if (!blob) return reject(new Error("Failed to get blob from canvas"));
...
const onSubmit = async () => {
  ...
  form.image =
    inputImage.value.length > 0
      ? [...new Uint8Array(await inputImage.value[0].raw!.arrayBuffer())]
      : undefined;
  form.mask =
    inputImage.value.length > 0
      ? [...new Uint8Array((await getMask())!)]
      : undefined;
  form.guidance = parseFloat(form.guidance as any);
  form.strength = parseFloat(form.strength as any);
  serverResponseCheck(
    fetch("https://ai.yun.ng/image", {
      method: "POST",
...
<template>
  ...
  <el-text>
    Model:
    <code>dreamshaper-8-lcm</code>
  </el-text>
  <br />
  <el-text>
    Model (in-paint):
    <code>stable-diffusion-v1-5-inpainting</code>
  </el-text>
</template>
