<script lang="ts" setup>
import { useDebounceFn } from '@vueuse/core'
import { projects, keywordsWeighted, cosSim, type ProjectDef, mean1axis } from "./projects";
const searching = ref(false);
const initialized = ref(false);
const search = ref('');
const vector = ref(false);
const searchEmbed = ref<number[]>([]);
const mapping = ref<{ [key: string]: number[] }>({});
const wordClick = (word: string) => {
  if (search.value === word.toLowerCase()) {
    search.value = '';
    return;
  }
  search.value = word.toLowerCase();
};
watch(vector, async () => {
  if (vector.value) {
    if (!initialized.value) {
      initialized.value = true;
      const vals = (await import('./out.json')).default;
      Object.entries(vals).forEach(([key, val]) => mapping.value[key] = val);
      return;
    }
  }
}, { once: true });

const searchVector = useDebounceFn(() => {
  console.log("searchVector called")
  if (!vector.value) return searching.value = false;
  if (!search.value) return searching.value = false;
  if (search.value.length < 3) return searching.value = false;
  if (mapping.value[search.value]) {
    searchEmbed.value = mapping.value[search.value];
    return searching.value = false;
  }
  return fetch('https://ai.yun.ng/embed', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    mode: 'cors',
    body: JSON.stringify({ text: search.value })
  }).then(res => res.json()).then(data => {
    if (!data.data) return searching.value = false;
    searchEmbed.value = data.data[0];
    mapping.value[search.value] = searchEmbed.value;
  }).finally(() => {
    searching.value = false;
  }).catch(err => {
    console.error(err);
    searching.value = false;
  });
}, 1500);
watch(search, async () => {
  if (vector.value) {
    searching.value = true;
    searchVector();
  }
});

function embedProject(project: ProjectDef) {
  return {
    desc: mapping.value[project.desc] ?? [],
    langs: project.langs.map(lang => mapping.value[lang] ?? []),
    keys: project.keys?.map(key => mapping.value[key] ?? []) ?? [],
  }
};
const filteredVectorized = computed(() => {
  if (!searchEmbed.value.length) return projects;
  return projects.map(p => {
    const embed = embedProject(p);
    const simD = cosSim(searchEmbed.value, embed.desc);
    const embedLang = mean1axis(embed.langs);
    const embedKey = mean1axis(embed.keys);
    const simL = cosSim(searchEmbed.value, embedLang);
    const simK = cosSim(searchEmbed.value, embedKey);
    const wD = 0.8, wL = 0.1, wK = 0.1;
    const overallScore = (wD * simD + wL * simL + wK * simK) / (wD + wL + wK);
    return {
      ...p,
      sim: overallScore,
      simD, simL, simK,
    }
  }).filter(p => p.sim > 0.3).sort((a, b) => b.sim - a.sim);
})
const filteredProjects = computed(() => {
  if (!search.value) return projects;
  if (vector.value) {
    return filteredVectorized.value;
  } else {
    const full = search.value.toLowerCase();
    const searchLowers = full.split(" ");
    return projects.filter(project => {
      return searchLowers.some(searchLower => {
        if (searchLower === "") return false;
        if (project.name.toLowerCase().includes(searchLower)) return true;
        if (project.desc.toLowerCase().includes(searchLower)) return true;
        if (project.langs?.some(lang => lang.toLowerCase() === searchLower)) return true;
        if (project.keys?.some(key => key.toLowerCase() === searchLower)) return true;
      }) || project.langs?.includes(full) || project.keys?.includes(full);
    });
  }
});
</script>
<template>
  <div class="skills-section">
    <Marquee @word-click="wordClick" class="fade-sides" :words="keywordsWeighted" :duration="100" :row-height="30"
      :angle="10" :selected="[search.toLowerCase()]" />
    <div
      class="skill-controls w-prose bg-black/80 -translate-x-1/2 -translate-y-1/2 top-50% left-50% absolute p-8 rounded max-w-full">
      <h1>Skills: Lookup Tool</h1>
      <i>&ldquo;Can you do X? Do you have experience with Y?"</i>
      <div class="relative">
        <input id="portfolio-search" placeholder="Type here..." v-model="search" autocomplete="off" />
        <span for="search-vector" @click="vector = !vector" :class="{ 'opacity-100': vector, 'opacity-30': !vector }"
          class="absolute top-50% left-2 cursor-pointer translate-y--50% text-xs select-none cursor-pointer transition-all">
          Vectorized (toggle)
        </span>
      </div>
      <Gallery class="scroll-container" :class="{ 'opacity-50': searching }">
        <div class="p-4 m-2 bg-black shadow rounded w-64 h-80 border-1 border-solid min-w-64 pt-0 relative project-card"
          :style="{ '--image': `url('${project.image}')` }" v-for="(project, index) in filteredProjects" :key="index">
          <small v-text="project.year" class="absolute opacity-50"></small>
          <small v-if="(project as any).sim" v-text="'cos-sim: ' + (project as any).sim.toFixed(2)"
            class="absolute opacity-50 right-0 border-solid border-1 border-white"></small>
          <h2 class="text-xl font-bold mb-1">{{ project.name }}</h2>
          <div class="max-h-42 overflow-y-auto scroll-container">
            <small class="text-xs opacity-50 text-balance">{{ project.desc }}</small>
          </div>
          <div class="flex flex-nowrap gap-1 mt-2 overflow-x-auto scroll-container select-none">
            <span v-for="(lang, index) in project.langs" :key="index" v-text="lang" @click="wordClick(lang)"
              class="px-1 py-0.5 text-xs border-solid rounded border-1 opacity-50 hover:opacity-100 cursor-pointer" />
          </div>
          <div class="flex flex-nowrap gap-1 mt-1 overflow-x-auto scroll-container select-none" v-if="project.keys">
            <span v-for="(key, index) in project.keys" :key="index" v-text="key" @click="wordClick(key)"
              class="px-1 py-0.2 text-xs border-solid rounded border-1 opacity-50 hover:opacity-100 cursor-pointer" />
          </div>
          <div class="flex flex-nowrap gap-2 mt-2 overflow-x-auto scroll-container select-none items-center">
            <a v-if="project.github" :href="'https://github.com/' + project.github" target="_blank">
              <svg width="20" height="20" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 98 96" fill="none">
                <path fill-rule="evenodd" clip-rule="evenodd"
                  d="M48.854 0C21.839 0 0 22 0 49.217c0 21.756 13.993 40.172 33.405 46.69 2.427.49 3.316-1.059 3.316-2.362 0-1.141-.08-5.052-.08-9.127-13.59 2.934-16.42-5.867-16.42-5.867-2.184-5.704-5.42-7.17-5.42-7.17-4.448-3.015.324-3.015.324-3.015 4.934.326 7.523 5.052 7.523 5.052 4.367 7.496 11.404 5.378 14.235 4.074.404-3.178 1.699-5.378 3.074-6.6-10.839-1.141-22.243-5.378-22.243-24.283 0-5.378 1.94-9.778 5.014-13.2-.485-1.222-2.184-6.275.486-13.038 0 0 4.125-1.304 13.426 5.052a46.97 46.97 0 0 1 12.214-1.63c4.125 0 8.33.571 12.213 1.63 9.302-6.356 13.427-5.052 13.427-5.052 2.67 6.763.97 11.816.485 13.038 3.155 3.422 5.015 7.822 5.015 13.2 0 18.905-11.404 23.06-22.324 24.283 1.78 1.548 3.316 4.481 3.316 9.126 0 6.6-.08 11.897-.08 13.526 0 1.304.89 2.853 3.316 2.364 19.412-6.52 33.405-24.935 33.405-46.691C97.707 22 75.788 0 48.854 0z"
                  fill="#fff" />
              </svg>
            </a>
            <a v-if="project.link" :href="project.link" target="_blank">
              <svg fill="#ffffff" height="20" width="20" version="1.1" id="Layer_1" v-if="project.link.endsWith('.jar')"
                xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink"
                viewBox="0 0 305.001 305.001" xml:space="preserve">
                <g id="XMLID_7_">
                  <path id="XMLID_8_" d="M150.99,56.513c-14.093,9.912-30.066,21.147-38.624,39.734c-14.865,32.426,30.418,67.798,32.353,69.288
		c0.45,0.347,0.988,0.519,1.525,0.519c0.57,0,1.141-0.195,1.605-0.583c0.899-0.752,1.154-2.029,0.614-3.069
		c-0.164-0.316-16.418-31.888-15.814-54.539c0.214-7.888,11.254-16.837,22.942-26.312c10.705-8.678,22.839-18.514,29.939-30.02
		c15.586-25.327-1.737-50.231-1.914-50.479c-0.688-0.966-1.958-1.317-3.044-0.84c-1.085,0.478-1.686,1.652-1.438,2.811
		c0.035,0.164,3.404,16.633-5.97,33.6C169.301,43.634,160.816,49.603,150.99,56.513z" />
                  <path id="XMLID_9_" d="M210.365,67.682c0.994-0.749,1.286-2.115,0.684-3.205c-0.602-1.09-1.913-1.571-3.077-1.129
		c-2.394,0.91-58.627,22.585-58.627,48.776c0,18.053,7.712,27.591,13.343,34.556c2.209,2.731,4.116,5.09,4.744,7.104
		c1.769,5.804-2.422,16.294-4.184,19.846c-0.508,1.022-0.259,2.259,0.605,3.005c0.467,0.403,1.05,0.607,1.634,0.607
		c0.497,0,0.996-0.148,1.427-0.448c0.967-0.673,23.63-16.696,19.565-36.001c-1.514-7.337-5.12-12.699-8.302-17.43
		c-4.929-7.329-8.489-12.624-3.088-22.403C181.419,89.556,210.076,67.899,210.365,67.682z" />
                  <path id="XMLID_10_" d="M63.99,177.659c-0.964,2.885-0.509,5.75,1.315,8.283c6.096,8.462,27.688,13.123,60.802,13.123
		c0.002,0,0.003,0,0.004,0c4.487,0,9.224-0.088,14.076-0.262c52.943-1.896,72.58-18.389,73.39-19.09
		c0.883-0.764,1.119-2.037,0.57-3.067c-0.549-1.029-1.733-1.546-2.864-1.235c-18.645,5.091-53.463,6.898-77.613,6.898
		c-27.023,0-40.785-1.946-44.154-3.383c1.729-2.374,12.392-6.613,25.605-9.212c1.263-0.248,2.131-1.414,2.006-2.695
		c-0.125-1.281-1.201-2.258-2.488-2.258C106.893,164.762,68.05,165.384,63.99,177.659z" />
                  <path id="XMLID_11_" d="M241.148,160.673c-10.92,0-21.275,5.472-21.711,5.705c-1.01,0.541-1.522,1.699-1.245,2.811
		c0.278,1.111,1.277,1.892,2.423,1.893c0.232,0.001,23.293,0.189,25.382,13.365c1.85,11.367-21.82,29.785-31.097,35.923
		c-1.002,0.663-1.391,1.945-0.926,3.052c0.395,0.943,1.314,1.533,2.304,1.533c0.173,0,0.348-0.018,0.522-0.056
		c2.202-0.47,53.855-11.852,48.394-41.927C261.862,164.541,250.278,160.673,241.148,160.673z" />
                  <path id="XMLID_12_" d="M205.725,216.69c0.18-0.964-0.221-1.944-1.023-2.506l-12.385-8.675c-0.604-0.423-1.367-0.556-2.076-0.368
		c-0.129,0.034-13.081,3.438-31.885,5.526c-7.463,0.837-15.822,1.279-24.175,1.279c-18.799,0-31.091-2.209-32.881-3.829
		c-0.237-0.455-0.162-0.662-0.12-0.777c0.325-0.905,2.068-1.98,3.192-2.405c1.241-0.459,1.91-1.807,1.524-3.073
		c-0.385-1.266-1.69-2.012-2.978-1.702c-12.424,2.998-18.499,7.191-18.057,12.461c0.785,9.343,22.428,14.139,40.725,15.408
		c2.631,0.18,5.477,0.272,8.456,0.272c0.002,0,0.003,0,0.005,0c30.425,0,69.429-9.546,69.819-9.643
		C204.818,218.423,205.544,217.654,205.725,216.69z" />
                  <path id="XMLID_13_"
                    d="M112.351,236.745c0.938-0.611,1.354-1.77,1.021-2.838c-0.332-1.068-1.331-1.769-2.453-1.755
		c-1.665,0.044-16.292,0.704-17.316,10.017c-0.31,2.783,0.487,5.325,2.37,7.556c5.252,6.224,19.428,9.923,43.332,11.31
		c2.828,0.169,5.7,0.254,8.539,0.254c30.39,0,50.857-9.515,51.714-9.92c0.831-0.393,1.379-1.209,1.428-2.127
		c0.049-0.917-0.409-1.788-1.193-2.267l-15.652-9.555c-0.543-0.331-1.193-0.441-1.813-0.314c-0.099,0.021-10.037,2.082-25.035,4.119
		c-2.838,0.385-6.392,0.581-10.562,0.581c-14.982,0-31.646-2.448-34.842-4.05C111.843,237.455,111.902,237.075,112.351,236.745z" />
                  <path id="XMLID_14_"
                    d="M133.681,290.018c69.61-0.059,106.971-12.438,114.168-20.228c2.548-2.757,2.823-5.366,2.606-7.07
		c-0.535-4.194-4.354-6.761-4.788-7.04c-1.045-0.672-2.447-0.496-3.262,0.444c-0.813,0.941-0.832,2.314-0.016,3.253
		c0.439,0.565,0.693,1.51-0.591,2.795c-2.877,2.687-31.897,10.844-80.215,13.294c-6.619,0.345-13.561,0.519-20.633,0.52
		c-43.262,0-74.923-5.925-79.079-9.379c1.603-2.301,12.801-5.979,24.711-8.058c1.342-0.234,2.249-1.499,2.041-2.845
		c-0.208-1.346-1.449-2.273-2.805-2.096c-0.336,0.045-1.475,0.115-2.796,0.195c-19.651,1.2-42.36,3.875-43.545,13.999
		c-0.36,3.086,0.557,5.886,2.726,8.324c5.307,5.963,20.562,13.891,91.475,13.891C133.68,290.018,133.68,290.018,133.681,290.018z" />
                  <path id="XMLID_15_" d="M261.522,271.985c-0.984-0.455-2.146-0.225-2.881,0.567c-0.103,0.11-10.568,11.054-42.035,17.48
		c-12.047,2.414-34.66,3.638-67.211,3.638c-32.612,0-63.643-1.283-63.953-1.296c-1.296-0.063-2.405,0.879-2.581,2.155
		c-0.177,1.276,0.645,2.477,1.897,2.775c0.323,0.077,32.844,7.696,77.31,7.696c21.327,0,42.08-1.733,61.684-5.151
		c36.553-6.408,39.112-24.533,39.203-25.301C263.082,273.474,262.504,272.44,261.522,271.985z" />
                </g>
              </svg>
              <svg fill="#ffffff" width="20" height="20" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" v-else>
                <path
                  d="M15.5 2.25a.75.75 0 01.75-.75h5.5a.75.75 0 01.75.75v5.5a.75.75 0 01-1.5 0V4.06l-6.22 6.22a.75.75 0 11-1.06-1.06L19.94 3h-3.69a.75.75 0 01-.75-.75z" />
                <path
                  d="M2.5 4.25c0-.966.784-1.75 1.75-1.75h8.5a.75.75 0 010 1.5h-8.5a.25.25 0 00-.25.25v15.5c0 .138.112.25.25.25h15.5a.25.25 0 00.25-.25v-8.5a.75.75 0 011.5 0v8.5a1.75 1.75 0 01-1.75 1.75H4.25a1.75 1.75 0 01-1.75-1.75V4.25z" />
              </svg>
            </a>
            <a v-if="project.video" :href="'https://www.youtube.com/watch?v=' + project.video" target="_blank">
              <svg fill="#ffffff" height="20" width="20" version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg"
                xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 310 310" xml:space="preserve">
                <g id="XMLID_822_">
                  <path id="XMLID_823_" d="M297.917,64.645c-11.19-13.302-31.85-18.728-71.306-18.728H83.386c-40.359,0-61.369,5.776-72.517,19.938
		C0,79.663,0,100.008,0,128.166v53.669c0,54.551,12.896,82.248,83.386,82.248h143.226c34.216,0,53.176-4.788,65.442-16.527
		C304.633,235.518,310,215.863,310,181.835v-53.669C310,98.471,309.159,78.006,297.917,64.645z M199.021,162.41l-65.038,33.991
		c-1.454,0.76-3.044,1.137-4.632,1.137c-1.798,0-3.592-0.484-5.181-1.446c-2.992-1.813-4.819-5.056-4.819-8.554v-67.764
		c0-3.492,1.822-6.732,4.808-8.546c2.987-1.814,6.702-1.938,9.801-0.328l65.038,33.772c3.309,1.718,5.387,5.134,5.392,8.861
		C204.394,157.263,202.325,160.684,199.021,162.41z" />
                </g>
              </svg>
            </a>
          </div>
        </div>
      </Gallery>
    </div>
  </div>
</template>

<style scoped>
.skills-section {
  z-index: 200;
  background-color: #000;
  position: relative;
  height: 100vh;
  width: 100%;
  border-top: 1px solid white;
  border-bottom: 1px solid white;
}

.fade-sides {
  --feather-size: 15%;
  -webkit-mask-image: linear-gradient(90deg,
      transparent 0%,
      black var(--feather-size),
      black calc(100% - var(--feather-size)),
      transparent 100%);
  mask-image: linear-gradient(90deg,
      transparent 0%,
      black var(--feather-size),
      black calc(100% - var(--feather-size)),
      transparent 100%);
}

.scroll-container::-webkit-scrollbar {
  background: transparent;
}

.scroll-container::-webkit-scrollbar-thumb {
  background: transparent;
  transition: background-color 0.3s ease;
}

.scroll-container:hover::-webkit-scrollbar-thumb {
  background: rgba(100, 100, 100, 0.7);
}

.scroll-container {
  scrollbar-width: thin;
  scrollbar-color: transparent transparent;
}

.scroll-container:hover {
  scrollbar-color: rgba(100, 100, 100, 0.7) transparent;
}

.project-card {
  transition: transform 0.3s ease;
  position: relative;
  z-index: 0;
}

.project-card::before {
  content: "";
  position: absolute;
  top: 0;
  left: 0;
  width: 100%;
  height: 100%;
  background-image: var(--image);
  background-size: cover;
  background-position: center;
  opacity: 0.3;
  filter: brightness(0.6);
  z-index: -1;
}

#portfolio-search {
  @apply w-full bg-black color-white text-center font-inherit text-xl border-0 border-b border-gray-500 focus:border-sky-500 outline-none mb-4px focus:mb-0 focus:border-b-5 transition-all;
  background-repeat: no-repeat;
  background-position: right center;
  background-size: 20px;
  background-image: url("data:image/svg+xml,%3Csvg%20width%3D%2220%22%20height%3D%2220%22%20fill%3D%22%23ffffff%22%20version%3D%221.1%22%20viewBox%3D%220%200%20488.4%20488.4%22%20xml%3Aspace%3D%22preserve%22%20xmlns%3D%22http%3A%2F%2Fwww.w3.org%2F2000%2Fsvg%22%3E%3Cpath%20d%3D%22m0%20203.25c0%20112.1%2091.2%20203.2%20203.2%20203.2%2051.6%200%2098.8-19.4%20134.7-51.2l129.5%20129.5c2.4%202.4%205.5%203.6%208.7%203.6s6.3-1.2%208.7-3.6c4.8-4.8%204.8-12.5%200-17.3l-129.6-129.5c31.8-35.9%2051.2-83%2051.2-134.7%200-112.1-91.2-203.2-203.2-203.2s-203.2%2091.1-203.2%20203.2zm381.9%200c0%2098.5-80.2%20178.7-178.7%20178.7s-178.7-80.2-178.7-178.7%2080.2-178.7%20178.7-178.7%20178.7%2080.1%20178.7%20178.7z%22%2F%3E%0A%3C%2Fsvg%3E")
}
</style>