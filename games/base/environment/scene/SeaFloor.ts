...
function SampleOceanHeight(x: number, y: number) {
...
function SampleHeight(x: number, y: number): number {
    const oceanHeight = SampleOceanHeight(x, y);
    const islandRadius = 100;
    const distanceFromCenter = Math.max(Math.sqrt((x - halfSize) ** 2 + (y - halfSize) ** 2) - 3, 0);
    if (distanceFromCenter >= islandRadius) {
        return oceanHeight;
    }
    const blendFactor = Math.pow(1 - distanceFromCenter / islandRadius, 1 / 4);
    return blendFactor * 1 + (1 - blendFactor) * oceanHeight;
}
