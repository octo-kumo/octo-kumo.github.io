export type PerlinNoise = (x: number, y: number, z: number, xWrap?: number, yWrap?: number, zWrap?: number) => number;
const quickNoise = (function () {
    'use strict';

    function buildTable(randFunc?: () => number) {
        if (!randFunc) randFunc = Math.random;
        // @NOTE(thom): could optimize this for allocations, but it
        // shouldn't be near anybody's fast path...
        var arr = new Array(256).fill(0).map(function (v, i) {
            return i;
        });
    ...
    function initTables(tab: number[] | (() => number), permTable: Uint8Array, gradTable: Int8Array) {
    ...
    function create(tab: number[] | (() => number)): PerlinNoise {
        ...
        function noise(x: number, y: number, z: number, xWrap?: number, yWrap?: number, zWrap?: number) {
