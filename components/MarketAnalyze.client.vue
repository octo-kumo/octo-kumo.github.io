...
function newtonsMethod(f: (x: number) => number, fPrime: (x: number) => number, target = 0, initialGuess = 0, tolerance = 1e-7, maxIterations = 1000) {
  let x = initialGuess;
  for (let i = 0; i < maxIterations; i++) {
    const y = f(x) - target;
    const yPrime = fPrime(x);

    if (Math.abs(yPrime) < tolerance) throw new Error('Derivative is too small, possible local extrema!');
...
function generateApexChartData(demandRelation: string, supplyRelation: string) {
  ...
  return [
    {
      name: 'Demand',
      data: demand, type: 'line',
    }, {
      name: 'Supply',
      data: supply, type: 'line',
    },
    {
      name: "CS",
